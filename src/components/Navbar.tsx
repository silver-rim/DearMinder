...
const Navbar = () => {
  ...
  return (
    <nav className="w-full py-4 px-4 md:px-8 bg-white/90 backdrop-blur-sm fixed top-0 z-50">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {user ? (
            <> {/* Logged-in links */} 
              ...
              <NavLink href="/settings" isButton variant="outline">
                <SettingsIcon className="mr-2 h-4 w-4" />
                Settings
              </NavLink>
              <NavLink href="/subscription" isButton variant="outline">
      ...
      {/* Mobile menu */} 
      {isMenuOpen && (
        <nav className="md:hidden absolute top-16 left-0 right-0 bg-white shadow-md py-4 px-6 z-50 animate-fade-in">
